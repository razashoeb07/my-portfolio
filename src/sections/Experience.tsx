...
export default function Experience() {
  ...
  return (
    <AnimatedSection id="experience">
      ...
      {/* Ye niche wala div missing tha aapke code mein, isliye error aa raha tha */}
      <div className="space-y-12">
        {experience?.map((job) => (
          <div key={`${job.company}-${job.role}`} className="timeline-item">
            <h3 className="text-xl font-bold">{job.role}</h3>
            ...
            {/* Project Name */}
            {job.project && (
              <p className="mt-4 font-semibold">Project: {job.project}</p>
            )}

            {/* Project Work */}
            {job.projectWork && (
              <ul className="mt-2 list-disc list-inside space-y-1">
                {job.projectWork.map((item: string, i: number) => (
                  <li key={i}>{item}</li>
                ))}
              </ul>
            )}
            ...
            {job.responsibilities && (
              <>
                <p className="mt-4 font-semibold">Roles & Responsibilities</p>
                <ul className="mt-2 list-disc list-inside space-y-1">
